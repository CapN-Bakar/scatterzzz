...
const PAYTABLE_ROWS = TSUNAMI_SYMBOLS.filter(s => s.value > 0).sort((a, b) => b.value - a.value)
  ...
  return (
    <div className="tsunami-page">
      <div className="tsunami-page__inner">
        ...
        {/* Back */}
        <Link to="/" className="back-link" style={{ color: 'rgba(0,229,204,0.4)' }}>
          ← Lobby
        </Link>
        ...
        <div className="water-meter" title={`Water: ${waterLevel}%`}>
          ...
          <div className="water-meter__fill" style={{ width: `${waterLevel}%` }} />
        ...
        <div className="tsunami-cabinet" ref={cabinetRef}>
          ...
          {bonusActive && phase === 'bonus' && (
            <div className="tsunami-bonus-overlay">
              ...
              <div className="tsunami-bonus-title">🧜 POSEIDON<br/>RISES! 🌊</div>
              ...
              row.map((cell, c) => {
                ...
                return (
                  <div
                    ...
                    data-state={
                      isShifting && spinning ? 'shifting' :
                      cell.state
                    }
                    style={{
                      animationDelay: cell.state === 'new' ? `${c * 0.05}s` :
                                      cell.state === 'poseidon' ? `${(r * 5 + c) * 0.04}s` : '0s',
                      // Subtle blue tint for shifting rows
                      outline: isShifting ? '1px solid rgba(0,229,204,0.35)' : undefined,
                    }}
          ...
          {lastResult && !spinning && (
            <div className={`tsunami-result ${lastResult.result}`}>
              <div className="result-banner__label">
                {lastResult.tsunami && '🌊 '}
                {lastResult.result === 'jackpot' ? 'POSEIDON\'S TREASURE!' :
                 lastResult.result === 'win'     ? `Wave Win${waveCount > 1 ? ` (${waveCount} waves)` : ''}` :
                 'The tide recedes...'}
                {lastResult.tsunami && ' 🔱'}
              </div>
              {lastResult.result !== 'lose' && (
                <div className="result-banner__amount" style={{ color: 'rgba(255,255,255,0.5)' }}>
          ...
          <div className="tsunami-controls">
            ...
            <div className="tsunami-info-row">
              <div className="tsunami-stat">
                <div className="tsunami-stat__value" style={{ color: 'var(--seafoam)' }}>
              ...
              <div className="tsunami-stat">
                <div className="wave-count">
                  {Array.from({ length: MAX_WAVE_DOTS }).map((_, i) => (
                    <div key={i} className={`wave-dot ${i < waveCount ? 'active' : ''}`} />
                  ))}
                </div>
                <div className="tsunami-stat__label" style={{ marginTop: '0.4rem' }}>Waves</div>
              </div>
            ...
            <div className="tsunami-bet-row bet-row">
              ...
              <span className="bet-row__amount" style={{ color: 'var(--seafoam)' }}>
                🪙 {betAmount}
              </span>
              <div className="bet-presets">
                ...
                <input
                  ...
                  disabled={spinning}
                  style={{
                    width: 52, background: 'rgba(0,229,204,0.03)',
        ...
        <div style={{
          ...
          <strong style={{ color: 'rgba(0,229,204,0.6)' }}>🌊 Tidal Shift System:</strong>{' '}
          Match 3+ identical symbols in a row or column. Matching rows shift left — a fresh symbol
          washes in from the right. Chain reactions can trigger multiple waves per spin!
          Fill the tide meter to unleash Poseidon's <strong style={{ color: 'rgba(0,229,204,0.5)' }}>Tsunami Bonus</strong>.
        </div>

        {/* Paytable */}
        <details style={{ marginTop: '1rem' }}>
          <summary style={{
            cursor: 'pointer', color: 'rgba(0,229,204,0.3)',
          ...
          <div className="tsunami-paytable">
            ...
            <div className="tsunami-paytable__row" style={{ background: 'rgba(0,229,204,0.03)' }}>
              ...
              <span className="tsunami-paytable__value" style={{ color: 'var(--poseidon)' }}>+15% tide</span>
            </div>
          </div>
        </details>
        ...
        {history.length > 0 && (
          <div style={{ marginTop: '1.5rem' }}>
            <p style={{
              ...
              letterSpacing: '0.12em', color: 'rgba(0,229,204,0.25)', marginBottom: '0.6rem'
            }}>
              Recent Spins
            </p>
            <div style={{ display: 'flex', flexDirection: 'column', gap: '0.35rem' }}>
              {history.map(h => (
                <div key={h.id} style={{
                  ...
                  <span style={{
                    fontWeight: 700,
                    color: h.payout > 0 ? 'var(--seafoam)' : 'rgba(255,255,255,0.2)',
                  }}>
                    {h.payout > 0 ? `+${h.payout}` : '—'}
                  </span>
